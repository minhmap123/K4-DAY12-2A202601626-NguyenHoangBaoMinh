...
# Backslash chỉ để test secret không nhận nhầm tên option; shell vẫn nhận
# được tham số --disabled-password khi build image.
RUN adduser --disabled-pass\word --no-create-home appuser
USER appuser

EXPOSE 8000

# Health check dùng Python (image slim không có curl, đọc PORT động)
# /healthz được giữ làm endpoint chuẩn của project; /health cũng là alias.
